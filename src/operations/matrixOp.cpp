...
#ifdef COMPILE_ISPC
    ispc::reduceMatrixISPC(m, v, row, col, reduced_dim);
#else
...
#ifdef COMPILE_ISPC
    ispc::eleSubtractISPC(alpha, beta, omega, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::sigmoidOpISPC(alpha, beta, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::eleSquareISPC(alpha, beta, row, col);
#else
...
/* It seems there's a little error
#ifdef COMPILE_ISPC
    ispc::reduceToValueISPC(alpha, value, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::linearOpISPC(alpha, beta, row, col, scale, bias);
#else
...
#ifdef COMPILE_ISPC
    ispc::multiEleISPC(alpha, beta, omega, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::multiEleInplaceISPC(alpha, beta, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::linearOpInplaceISPC(alpha, beta, row, col, a, b, c);
#else
...
#ifdef COMPILE_ISPC
    ispc::UpdateWeightWithRegISPC(delta, weight, row, col, lr, reg);
#else
...
#ifdef COMPILE_ISPC
    return ispc::getCorrectlyRecognizedISPC(predicts, targets, row, col);
#else
...
#ifdef COMPILE_ISPC
    ispc::softmaxISPC(alpha, beta, row, col);
#else
...
#ifdef COMPILE_ISPC
    return ispc::negLogLikelihoodISPC(predicts, targets, row, col);
#else
